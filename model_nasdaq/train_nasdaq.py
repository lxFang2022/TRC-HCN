...
class ReRaLSTM:
    ...
    def clustersHyperedge(self, hedge_nums = 400):
        ...
        # data = np.load("data400_5.npy")
        # row = np.load("row400_5.npy")
        # col = np.load("col400_5.npy")+
    ...
    def train(self,iii):
        ...
        batch_offsets = np.arange(start=0, stop=self.valid_index-4, dtype=int)
        ...
        for i in range(self.epochs):
            ...
            for j in tqdm(range(self.valid_index - self.parameters['seq'] - self.steps + 1)):
                emb_batch, mask_batch, price_batch, gt_batch = self.get_batch(
                    batch_offsets[j])
                ########  emb_batch:1026*4*5 -> updown:1026*4
                # print("batch_offsets[j]+4:",batch_offsets[j]+4)
                optimizer_hgat.zero_grad()
                output = model(torch.FloatTensor(emb_batch).to(device), hyp_input_T[batch_offsets[j]:batch_offsets[j]+4],hyp_input)  # , hyp_input
            ...
            with torch.no_grad():
                ...
                for cur_offset in range(
                    ...
                    output_val = model(torch.FloatTensor(emb_batch).to(device), hyp_input_T[batch_offsets[j]:batch_offsets[j]+4],hyp_input)  # , hyp_input
                ...
                for cur_offset in tqdm(range(self.test_index - self.parameters['seq'] - self.steps + 1,
                                             self.trade_dates - self.parameters['seq'] - self.steps + 1)):
                    emb_batch, mask_batch, price_batch, gt_batch = self.get_batch(cur_offset)

                    output_test = model(torch.FloatTensor(emb_batch).to(device), hyp_input_T[batch_offsets[j]:batch_offsets[j]+4],hyp_input)  # , hyp_input
                ...
                # trainloss.append(tra_rank_loss / (self.valid_index - self.parameters['seq'] - self.steps + 1))
                # valloss.append(cur_valid_perf['mse'])
                # testloss.append(cur_test_perf['mse'])
                # # valid_ndcg.append(cur_valid_perf['ndcg_score_top5'])
                # valid_irr1.append(cur_valid_perf['btl'])
                # valid_irr5.append(cur_valid_perf['btl5'])
                # valid_mrr.append(cur_valid_perf['mrrt'])
                # valid_sharpe1.append(cur_valid_perf['sharpe1'])
                # vaild_sharpe.append(cur_valid_perf['sharpe5'])
                # # test_ndcg.append(cur_test_perf['ndcg_score_top5'])
                # test_irr1.append(cur_test_perf['btl'])
                # test_irr5.append(cur_test_perf['btl5'])
                # test_mrr.append(cur_test_perf['mrrt'])
                # test_sharpe1.append(cur_test_perf['sharpe1'])
                # test_sharpe5.append(cur_test_perf['sharpe5'])

            gc.collect()
            torch.cuda.empty_cache()
        # fig, ax = plt.subplots(7, 2)
        #
        # ax[0][0].plot(epochs_value, trainloss, label="Train Loss")
        # ax[0][0].xaxis.set_ticks_position('bottom')
        #
        # ax[1][0].plot(epochs_value, valloss, label="Validation loss")
        # ax[1][0].xaxis.set_ticks_position('bottom')
        #
        # ax[2][0].plot(epochs_value, testloss, label="Test loss")
        # ax[2][0].xaxis.set_ticks_position('bottom')
        #
        # ax[3][0].plot(epochs_value, valid_mrr, label="Vaild MRR")
        # ax[3][0].xaxis.set_ticks_position('bottom')
        #
        # ax[4][0].plot(epochs_value, valid_sharpe1, label="Vaild shape1")
        # ax[4][0].xaxis.set_ticks_position('bottom')
        #
        # ax[5][0].plot(epochs_value, valid_irr1, label="Vaild IRR1")
        # ax[5][0].xaxis.set_ticks_position('bottom')
        #
        # ax[6][0].plot(epochs_value, valid_irr5, label="Vaild IRR5")
        # ax[6][0].xaxis.set_ticks_position('bottom')
        #
        # ax[0][1].plot(epochs_value, vaild_sharpe, label="Vaild Sharpe5")
        # ax[0][1].xaxis.set_ticks_position('bottom')
        #
        # ax[1][1].plot(epochs_value, test_mrr, label="Test MRR")
        # ax[1][1].xaxis.set_ticks_position('bottom')
        #
        # ax[2][1].plot(epochs_value, test_sharpe1, label="Test sharpe1")
        # ax[2][1].xaxis.set_ticks_position('bottom')
        #
        # ax[3][1].plot(epochs_value, test_irr1, label="Test IRR1")
        # ax[3][1].xaxis.set_ticks_position('bottom')
        #
        # ax[4][1].plot(epochs_value, test_irr5, label="Test IRR5")
        # ax[4][1].xaxis.set_ticks_position('bottom')
        #
        #
        # ax[5][1].plot(epochs_value, test_sharpe5, label="Test Sharpe5")
        # ax[5][1].xaxis.set_ticks_position('bottom')


        # ax[0][0].set_title("Loss Figure")
        ...
        # string = './loss_img/' + str(iii+6) + ' Loss Figure.jpg'
        # plt.savefig(string)
        # plt.show()


        print("训练最好的epoch:", maxtest_btlindex)
        print("最好的IRR1:", maxtest_btl)
        plt.rcParams['figure.figsize'] = (12.0, 8.0)
        plt.plot(btl_[maxtest_btlindex], label="Top1 IRR")
        np.save("TDHCN_NASDAQ.npy", btl_[maxtest_btlindex])
        plt.legend()
        string = './loss_img/' + str(iii+6) + ' Top1 IRR.jpg'
        plt.savefig(string)
        plt.show()
...
if __name__ == '__main__':
    ...
    for i in range(len(lr)):
        ...
        parser.add_argument('-r', default=lr[i],
                            help='learning rate')
        ...
        RR_LSTM = ReRaLSTM(
            ...
            steps=1, epochs=2, batch_size=None, gpu=args.gpu,
            in_pro=args.inner_prod
        )

        pred_all = RR_LSTM.train(i)
